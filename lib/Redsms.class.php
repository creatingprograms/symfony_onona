...
class Redsms
{
    ...
    protected function getCurlResult($curlResource)
    {
        ...
        file_put_contents($_SERVER['DOCUMENT_ROOT']. '/../websms.log', print_r([$info, $responseArray], true), FILE_APPEND);

        if (json_last_error() != JSON_ERROR_NONE) {
            $responseArray['errors'] = 'Ошибка разбора ответа сервера. Код http: ' . $info['http_code'];
            // throw new \Exception('Error response format', $info['http_code']);
        }
        
        if ($info['http_code'] != 200) {
            $responseArray['errors'] = $responseArray['error_message'] . '. Код http: ' . $info['http_code'];
