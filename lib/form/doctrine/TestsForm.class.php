...
class TestsForm extends BaseTestsForm {
    ...
    public function configure() {
        ...
        //$this->embedForm('new_Question', $newQuestionForm);

        /* $newQuestion = new TestsQuestion();
          $newQuestion->setTestId($this->object);
          $newQuestionForm = new TestsQuestionForm($newQuestion);
          $this->embedFormForEach('new_Question', $newQuestionForm, 1);
         */
        ...
        // $newQuestionForm = new TestsQuestionForm();

        $this->embedRelation('Question');

        $this->widgetSchema['Question']->setLabel('Вопросы');
...
//echo "1";
        ...
        $this->embedRelation('Result');
        $this->widgetSchema['Result']->setLabel('Результаты');
        $relationName = "Result";
        $relationSettings = array(
            'considerNewFormEmptyFields' => array('balls_to', 'balls_from', 'results'),
            'noNewForm' => false,
            'newFormLabel' => 'Новый результат',
            'newFormClass' => 'TestsResultForm',
            'newFormClassArgs' => array(array('sf_user' => $this->getOption('sf_user'))),
            'displayEmptyRelations' => false,
            'formClass' => 'TestsResultForm',
            'formClassArgs' => array(array('ah_add_delete_checkbox' => true)),
            'newFormAfterExistingRelations' => true,
            'formFormatter' => null,
            'multipleNewForms' => true,
        ...
        if ($this->isNew) {
            ...
            $this->setWidget('Question', new sfWidgetFormInputHidden(array(), array('value' => false)));
            $this->setValidator("Question", new sfValidatorBoolean(array('required' => false)));
            $this->setWidget('Result', new sfWidgetFormInputHidden(array(), array('value' => false)));
            $this->setValidator("Result", new sfValidatorBoolean(array('required' => false)));
        }
        ...
        if (isset($values['Question'])) {
            foreach ($values['Question'] as $key => $question) {
                ...
                foreach ($values['Question'][$key]['new_Answer'] as $newAnswer) {
                    if (@$newAnswer['answer'] != "") {
                        $newAnswerObject = new TestsQuestionAnswer();
                        $newAnswerObject->setTestquestionId($values['Question'][$key]['id']);
    ...
    protected function doUpdateObject($values) {
        ...
        if (count($this->numbersToDelete)) {
            foreach ($this->numbersToDelete as $index => $id) {
                ...
                TestsQuestionTable::getInstance()->findOneById($id)->delete();
            }
        }

        if (count($this->answerToDelete)) {
            foreach ($this->answerToDelete as $index => $id) {
                TestsQuestionAnswerTable::getInstance()->createQuery()->delete()->where("id=?", $id['id'])->execute();
            }
        }
        ...
        if (count($this->numbersToDeleteResult)) {
            foreach ($this->numbersToDeleteResult as $index => $id) {
                ...
                TestsResultTable::getInstance()->findOneById($id)->delete();
            }
        }
        /* print_r($values);
          exit; */
            ...
            if ($form instanceof sfFormObject) {
                if (!in_array($form->getObject()->getId(), $this->numbersToDelete)) {
                    ...
                    if ($form->getObject()->isNew()) {
                        ...
                        if (get_class($formQuestion) != "TestsResult")
                            if ($formQuestion->getQuestion() != "") {
                                //print_r($formQuestion->Answer[1]->getId());exit;
                                $formQuestion->setTestId($this->object);
                                ...
                                foreach ($valuesAll['new_Question'][$formKey]['new_Answer'] as $newAns) {
                                    //print_r($newAns);
                                    $newAnsObj = new TestsQuestionAnswer();
                                    $newAnsObj->setAnswer($newAns['answer']);
                                    $newAnsObj->setBalls($newAns['balls']);
                                    $newAnsObj->setTestquestionId($formQuestion);
                                    $newAnsObj->save();
