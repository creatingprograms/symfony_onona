...
class TestsQuestionForm extends BaseTestsQuestionForm {
    ...
    public function configure() {
        ...
        $this->embedRelation('Answer', null, array('orderBy' => array('position', 'ASC')));

        /* $newAnswer = new TestsQuestionAnswer();
          $newAnswer->setTestquestionId($this->object);
          $newAnswerForm = new TestsQuestionAnswerForm($newAnswer);

          $newAnswerForm->getWidgetSchema()->setLabel('Новый ответ'); */
        $this->widgetSchema['Answer']->setLabel('Ответы');
        $this->widgetSchema['question']->setLabel('Вопрос');
        $this->widgetSchema['number']->setLabel('Номер вопроса');


        /* $this->embedForm('new_Answer', $newAnswerForm);
...
//print_r($this->getOption('numForm'));echo"r";
        //print_r($_POST['tests']['new_Question'][$this->getOption('numForm') - 1]['new_Answer']);
        ...
        /*echo $this->getOption('numForm') - 1;
        print_r($_POST['tests']['new_Question'][$this->getOption('numForm') - 1]);*/
        if (isset($_POST['tests']['new_Question'][$this->getOption('numForm') - 1])) {
            //echo count($_POST['tests']['new_Question'][$this->getOption('numForm') - 1]['new_Answer']);
            for ($i = 0; $i < count($_POST['tests']['new_Question'][$this->getOption('numForm') - 1]['new_Answer']); $i++) {
...
//print_r($this->are);
        ...
        $this->mergePostValidator(new TestQuestionAnswerValidatorSchema());
    }
    ...
    protected function doUpdateObject($values) {
        ...
        if (count($this->answerToDelete)) {
            foreach ($this->answerToDelete as $index => $id) {
                ...
                TestsQuestionAnswerTable::getInstance()->findOneById($id)->delete();
            }
        }
