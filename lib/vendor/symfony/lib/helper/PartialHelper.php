 ...
 * @see    end_slot
 */
function JSInPages($value = null) {
    $context = sfContext::getInstance();
    $response = $context->getResponse();

    $slot_names = sfConfig::get('symfony.view.slot_names', array());
    if (in_array("JSInPages", $slot_names)) {
        throw new sfCacheException(sprintf('A slot named "%s" is already started.', "JSInPages"));
    }

    if (sfConfig::get('sf_logging_enabled')) {
        $context->getEventDispatcher()->notify(new sfEvent(null, 'application.log', array(sprintf('Set slot "%s"', "JSInPages"))));
    }

    if (null !== $value) {
        $response->setSlot("JSInPages", get_slot("JSInPages") . $value);

        return;
    }

    $slot_names[] = "JSInPages";

    $response->setSlot("JSInPages", '');
    sfConfig::set('symfony.view.slot_names', $slot_names);
