...
class sfWidgetFormInputFileEditable extends sfWidgetFormInputFile {
    ...
    public function render($name, $value = null, $attributes = array(), $errors = array()) {
        ...
        return strtr($this->getOption('template'), array('%input%' => $input, '%delete%' => $delete, '%delete_label%' => $deleteLabel, '%file%' => $this->getFileAsTag($attributes), '%fileInfo%' => $this->getFileInfo()));
    }
    ...
    protected function getFileInfo() {
        if ($this->getOption('is_image')) {
            $addr = explode("/", $this->getOption('file_src'));
            if (end($addr) != "") {
                if (file_exists(sfConfig::get('sf_upload_dir') . '/photo/thumbnails_250x250/' . end($addr))) {
                    $images = new sfImage(sfConfig::get('sf_upload_dir') . '/photo/thumbnails_250x250/' . end($addr));
                    $images2 = new sfImage(sfConfig::get('sf_upload_dir') . '/photo/' . end($addr));
                    //echo $images->getHeight();
                    //echo $images->getWidth();
                    //echo (round(filesize(sfConfig::get('sf_upload_dir') . '/photo/thumbnails_250x250/' .end($addr))/1024));

                    return "<div><b>Информация по эскизу:</b> <br>Размер: " . $images->getWidth() . "x" . $images->getHeight() . "<br> Весит: " . (round(filesize(sfConfig::get('sf_upload_dir') . '/photo/thumbnails_250x250/' . end($addr)) / 1024)) . "КБ<br><br>
          <b><a href=\"/uploads/photo/" . end($addr) . "\">Информация по основному размеру</a>:</b> <br>Размер: " . $images2->getWidth() . "x" . $images2->getHeight() . "<br> Весит: " . (round(filesize(sfConfig::get('sf_upload_dir') . '/photo/' . end($addr)) / 1024)) . "КБ</div>";
                } elseif (file_exists(sfConfig::get('sf_upload_dir') . '/photouser/thumbnails/' . end($addr))) {
                    $images = new sfImage(sfConfig::get('sf_upload_dir') . '/photouser/thumbnails/' . end($addr));
                    $images2 = new sfImage(sfConfig::get('sf_upload_dir') . '/photouser/' . end($addr));
                    //echo $images->getHeight();
                    //echo $images->getWidth();
                    //echo (round(filesize(sfConfig::get('sf_upload_dir') . '/photo/thumbnails_250x250/' .end($addr))/1024));

                    return "<div><b>Информация по эскизу:</b> <br>Размер: " . $images->getWidth() . "x" . $images->getHeight() . "<br> Весит: " . (round(filesize(sfConfig::get('sf_upload_dir') . '/photouser/thumbnails/' . end($addr)) / 1024)) . "КБ<br><br>
          <b><a href=\"/uploads/photouser/" . end($addr) . "\" target=\"_blank\">Информация по основному размеру</a>:</b> <br>Размер: " . $images2->getWidth() . "x" . $images2->getHeight() . "<br> Весит: " . (round(filesize(sfConfig::get('sf_upload_dir') . '/photouser/' . end($addr)) / 1024)) . "КБ</div>";
                }
            }
        } else {
            return $this->getOption('file_src');
