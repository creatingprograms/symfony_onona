...
class photosTask extends sfBaseTask {
...
The [yamarket|INFO] Формирует файл csv формата артикул - ссылка на фото.
Call it with:

  [php symfony photos|INFO]
EOF;
    }
    ...
    protected function execute($arguments = array(), $options = array()) {
        ...
        */
        ...
        $sqlBody="SELECT id, code, name, slug, price, old_price, `count`  FROM product ";
        $result = $q->execute($sqlBody);
        $result->setFetchMode(Doctrine_Core::FETCH_ASSOC);
        $tmp = $result->fetchAll();
        unset($resultArray);
        $resultArray[]=implode($delimiter, ['id', 'Артикул', 'Название', 'Ссылка', 'Цена', 'Старая цена', 'Количество на складе']);
        foreach ($tmp as $value) {
