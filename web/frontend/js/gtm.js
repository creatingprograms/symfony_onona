$(document).ready(function(){
  ...
  function initGTM(){
    ...
    $(document).on('click', ".js-navy", function(){//Клик по ехать с наигатором
      try {
        ym(144683,'reachGoal','ekhat-yandeks-navigator')
        console.log('ekhat-yandeks-navigator reach goal send success');
      } catch (e) {
        console.log('ekhat-yandeks-navigator send to yandex error');
      }
    });
    ...
    $(document).on('click', '.gtm-detail-add-to-basket', function(){ //кладем в корзину из детальной
      ...
      var product={
        ...
        'brand': $page.data('manufacturer'),         // название бренда.
    ...
    if($('#gtm-thank-you').length){ //страница Спасибо
      var
        $div=$('#gtm-thank-you'),
        id=$div.data('id'),
        revenue=$div.find('.gtm-thank-you-total').data('total'),
        shipping=$div.find('.gtm-thank-you-delivery').data('delivery'),
    ...
    $('.gtm-decrement-in-cart').on('click', function(){ //Уменьшение количества в корзине
      var
        $row   =  $(this).closest('.basket-item').find('.gtm-checkout-item'),
        quant  = 1;
  ...
  function sendThankYou(actionField, products, promocode){//Страница спасибо
    if(isTest){
      ...
      console.log([actionField, products,promocode]);
