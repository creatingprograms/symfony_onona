...
abstract class BasesfGuardForgotPasswordActions extends sfActions {
    ...
    public function executeIndex($request) {
        ...
        // die(__FILE__.'|'.__LINE__.'|'.$class);
        $this->noneMail = false;

        if ($request->isMethod('post')) {
            $this->form->bind($request->getParameter($this->form->getName()));

            if ($this->form->isValid() == "noneMail") {
                $this->noneMail = true;
            }
            //echo (string) $this->form->isValid() != "noneMail";
            if ($this->form->isValid() and (string) $this->form->isValid() != "noneMail") {
                ...
                $message = Swift_Message::newInstance()
                        ...
                        ->setTo(trim($this->form->user->email_address))
                        ->setSubject('Восстановление пароля'/* .$this->form->user->username */)
                ...
                $this->setTemplate('password_mail');
            } else {
        ...
        if ($request->isMethod('post')) {
            ...
            if ($this->form->isValid()) {
                ...
                $message = Swift_Message::newInstance()
                        ...
                        ->setSubject('Новый пароль для  ' . $this->user->email_address)
