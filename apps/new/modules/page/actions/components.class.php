...
class pageComponents extends sfComponents {
    ...
    public function executePopup(sfWebRequest $request){//Выводит всплывающие формы

    }
    public function executeForms(sfWebRequest $request){//Выводит формы
      if($this->type="notify"){
        if($this->getUser()->isAuthenticated()) {
          $this->email = $this->getUser()->getGuardUser()->get('email_address');
          $this->fio = $this->getUser()->getGuardUser()->get('first_name');
        }
      }
    }

    public function executeFirstvisit(sfWebRequest $request){//Выводит форму подписки в футере

    }

    public function executeRrMainBlock(sfWebRequest $request){//Выводит блок RR на главной
      $this->isEnabled=csSettings::get('rr_main_enabled')=='on';
    }

    public function executeBannerNew(sfWebRequest $request){//Выводит блок баннеров
      // $prod=Doctrine_Core::getTable('Product')->findOneById(762);
      // die('|'.(null!==$prod->getBonus()).'|');
      if(empty($this->variant)) $this->variant="double";

      $banners=BannersTable::getInstance()
        ->createQuery()
        ->where("is_public=1 AND position='".$this->type."'");
      if($this->variant=="double") {
        $banners->limit(2);
      }
      $this->banners=$banners->execute();
    }

    public function executeBanner(sfWebRequest $request){//Выводит страницу
      $banner=Doctrine_Core::getTable('Banners')->findOneByPosition($this->type);
      if(is_object($banner) && !$banner->getIsPublic()) $banner=false;
    ...
    public function executeGetFortune(sfWebRequest $request){//Колесо удачи
      $this->needToShow = !$_COOKIE['no-show-wheel-1'];
      $nextShow = 20 + time();
      if(!empty($_SESSION['fortune_next_show']) && $_SESSION['fortune_next_show'] > time()) $nextShow = $_SESSION['fortune_next_show'];
      $this->nextShow = $nextShow;
      /*
      $this->needToShow = false;
      if($this->getUser()->isAuthenticated()){
        $id = $this->getUser()->getGuardUser()->get('id');
        if(in_array($id, [177106, 336675]))
    ...
    public function executeShopsList(sfWebRequest $request){//Список магазинов
        if(!$this->is_krasnodar){
            $shops=ShopsTable::getInstance()
                ->createQuery()
                ->where('is_active=1 and city_id IN('.implode(', ', $this->city_id).')')->orderBy('metro');
        ...
        if(($this->type=='mobile_mod' || $this->type=='mobile') && count($this->shops)) {
          foreach ($this->shops as $shop) {
            if(empty($metroList[$shop->getMetro()]))
              $metroList[$shop->getMetro()]=[
                'name' => $shop->getMetro(),
                'slug' => mb_strtolower($shop->getSlug()),
              ];
            if(empty($metroList[$shop->getMetro()]['image'])){
              ...
              if(file_exists($_SERVER['DOCUMENT_ROOT'].$img)) $metroList[$shop->getMetro()]['image']=$img;
            }
          }
    ...
    private function getCatalogs($includeNovice=true){
        ...
        foreach ($menu as $link){
            ...
            $banner=Doctrine_Core::getTable('Banners')->findOneByPosition($link->getSlug());
            ...
            $subMenu[]=[
                ...
                'banner' => [
                  'link' => $banner->getHref(),
                  'img' => "/uploads/media/".$banner->getSrc(),
                ],
            ];

            if($menuItemLink == '/catalog/preparaty')
                $subMenu[] = [
                    'link' => '/category/podarochnye-nabory',
        ...
        if($includeNovice){
          $banner=Doctrine_Core::getTable('Banners')->findOneByPosition('dlya_novichkov');
          $subMenu[]=[
        ...
        $includeCerts=true;
        if($includeCerts){
          $banner=Doctrine_Core::getTable('Banners')->findOneByPosition('certs');
          $subLine=[
              'link' => '/category/podarochnye-sertifikaty',
              'name' => 'Подарочные сертификаты',
              'is_target_blank' => false,
              'is_noindex' => false,
              'name_part' => 'Подарочные сертификаты',
              'name_part_2' => '',
              'class' => '-certs',
              'submenu' => [
                  [
    ...
    private function getSubCatsById($categoryId){//Подразделы категории
        ...
        return isset($subMenu) ? $subMenu : null;
    }
    ...
    private function getSubCats($catalogSlug, $thirdLevel=false){
        ...
        foreach ($categorys as  $value) {
            ...
            if($thirdLevel) {
                ...
                if(is_array($tmp['submenu'])) foreach ($tmp['submenu'] as $currentSubmenuItem) {
                    if ($currentSubmenuItem['isActive']) {
                        $tmp['isActive'] = true;
                        break;
                    }
    ...
    private function getSubMenu($menuId, $positionmenu='main_frontend'){
        ...
        foreach ($menu as $link){
            $tmp=[
                'link' => mb_strtolower($link->getUrl()),
                'name' => $link->getText(),
                'counter_target' => $link->getConterTarget(),
                'is_target_blank' => $link->getTargetBlank(),
            ];
            $tmp['is_noindex']=false;
            if(false !== stripos($tmp['link'], '//')) $tmp['is_noindex']=true;//Если есть двойная косая - ссылка внешняя
    ...
    public function executeMobileMenu(sfWebRequest $request){
        ...
        $this->menuBot=isset($botMenu) ? $botMenu : null;
    }
    ...
    public function executeTopMenuMainNewNew(sfWebRequest $request){ //Новая итерация меню
        $menu = MenuTable::getInstance()->createQuery()->where("positionmenu = 'main_frontend'")->addWhere("is_active = 1")->addWhere("parents_id = 0 OR parents_id iS NULL")->execute();

        foreach ($menu as $link){
            $url=$link->getUrl();
            if($url=='/catalog'){
              $banner=Doctrine_Core::getTable('Banners')->findOneByPosition('catalog');
              $tmp=[
                  'link' => mb_strtolower($url),
                  'name' => $link->getText(),
                  'submenu' => $this->getCatalogs(),
    ...
    public function executeProductsReviews(sfWebRequest $request){//Обзоры продуктов
      $this->show=true;
      $this->show=false;
    }

    public function executeTopNews(sfWebRequest $request){//Блок новостей
    ...
    public function executeItemsReviews(sfWebRequest $request){//Обзоры товаров
      //Не реализовано
    }
    public function executeLastReviews(sfWebRequest $request){//Последние отзывы
      $limit = 12;
      $where = "is_onmain='1'";
      if(!empty($this->cat_id)) {
        $cat_id = $this->cat_id;
        $sqlBody = "SELECT `product_id` FROM `category_product` WHERE `category_id` = $cat_id";
        $q = Doctrine_Manager::getInstance()->getCurrentConnection();
        $ids = $q->execute($sqlBody)->fetchAll(Doctrine_Core::FETCH_ASSOC);
        if(!empty($ids) && is_array($ids)) 
            foreach($ids as $id) 
                $whereIds[] = $id['product_id'];
        $whereIds[] = -1;
        $where = 'product_id IN('. implode(', ', $whereIds) .')';
        $limit = 25;
      }
      if(!empty($this->is_novice) && $this->is_novice) $where .= " AND is_novice = 1";
      ...
      $comments = CommentsTable::getInstance()
          ...
          ->orderBy("sort_index desc, created_at desc")
          ->limit($limit)
          ->execute();
      ...
      foreach ($comments as $comment) {
        $photos = PhotoTable::getInstance()
          ->createQuery()
          ->where("album_id=(select photoalbum_id from product_photoalbum where product_id=" . $comment->getProductId() . " limit 0,1)")
          ->orderBy("position")
          ->execute();
        // $this->photo=false;
        if(sizeof($photos)){
          $photoAlbum[$comment->getProductId()]="/uploads/photo/thumbnails_250x250/".$photos[0]->getFilename();
        }
        else $photoAlbum[$comment->getProductId()]=false;
      }
    ...
    public function executeSliderActions(sfWebRequest $request){//Слайдер с акциями
      $slides = SlidersTable::getInstance()
          ->createQuery()
          ->where("is_active='1'")
          ->andWhere("positionpage='sl_actions'")
    ...
    public function executeBlockSliders(sfWebRequest $request){//Слайдер на главной и ему подобные
        ...
        $this->slides = $slides;
        // echo "<pre>";var_dump([!$this->moscow, count($this->slides)]);die();
    }
    ...
    public function executeSdek(sfWebRequest $request) {//Выводит калькулятор сети доставки СДЭК
    }

    public function executePostMap(sfWebRequest $request) {//Выводит калькулятор Почты России
    }

    public function executeSberMap(sfWebRequest $request) {//Выводит калькулятор СберЛогистика
    }

    public function executeViaMap(sfWebRequest $request) {//Выводит калькулятор Via
    }
