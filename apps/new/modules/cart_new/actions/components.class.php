...
class cart_newComponents extends sfComponents
{
  ...
  public function executeImeform(sfWebRequest $request) { //Форма оплаты  iMe wallet
    die('Требует переработки! Need to be rewrite');
    global $isTest;
  ...
  public function executeYookassaform(sfWebRequest $request) { //Форма оплаты Yookassa
    ...
    if(!is_object($ordPaym) || $ordPaym->status!='succeeded'){//Если объекта еще нет или статус у него не оплачен
      ...
      $amount=0;
      foreach ($products_old as $key => $productInfo)//считаем сумму
        $amount+=(!empty($productInfo['price_final']) ? $productInfo['price_final'] : $productInfo['price_w_discount'])*$productInfo['quantity'];
      // (($productInfo['discount']==100 || $productInfo['price_w_discount'] > 0) ? $productInfo['price_w_discount'] : $productInfo['price'])*$productInfo['quantity'];
      // die('<pre>!~'.print_r([$amount, $products_old] , true));
      // $amount-=$this->order->getBonuspay();
      ...
      $confirmUrl='https://'.($isTest ? 'dev.' : '').'onona.ru/customer/orderdetails/'.$order->getId().'?payed=Y';
  ...
  public function executeMkbform(sfWebRequest $request) { //Форма оплаты МКБ
    ...
    $amount=0;
    foreach ($products_old as $key => $productInfo){ //считаем сумму
      if(isset($productInfo['price_final'])) $amount+= $productInfo['price_final']*$productInfo['quantity'];
      else $amount+=(($productInfo['discount']==100 || $productInfo['price_w_discount'] > 0) ? $productInfo['price_w_discount'] : $productInfo['price'])*$productInfo['quantity'];}
  ...
  public function executeTopCart(sfWebRequest $request) {//Корзина в шапке
    ...
    if(is_array($products_old)) foreach ($products_old as $product) {
    ...
    if(!isset($this->type)) $this->type = false;
  }
