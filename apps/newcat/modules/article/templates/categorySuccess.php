...
/* if($category){
  ...
  slot('metaTitle', $categoryName);
  slot('metaKeywords', $categoryName);
  slot('metaDescription', $categoryName);
  } */
if ($category) {
    $startTitle = $category->getName();
    slot('canonicalSlugArticlecategory', $category->getSlug());
}else
...
<ul class="breadcrumbs">
    ...
    <?if( $catalog[0]->getName() != 'Сексопедия'){?>
        <li><a href="/sexopedia<?= "/catalog/" . $catalog[0]->getSlug() ?>"><?= $catalog[0]->getName() ?> </a></li><? }} ?>
    <li><?php if ($category) echo $category->getName(); else echo $categoryName; ?> </li>
</ul>
<h1 class="title"><?php if ($category) echo $category->getName(); else echo $categoryName; ?></h1>
<?php
...
foreach ($articles as $article):
    ...
    <div class="divArticleHover">
        <div class="preview-image"><?php if ($article->getImg() != "") { ?><a href="/sexopedia/<?= $article->getSlug() ?>"><img class="thumbnails" src="/images/pixel.gif" data-original="/uploads/photo/<?= $article->getImg() ?>" alt="<?= str_replace(array("'", '"'), "",$article->getName()) ?>" style="margin: 0 10px;"/></a><?php } else { ?>&nbsp; <?php } ?></div>
        <div>
