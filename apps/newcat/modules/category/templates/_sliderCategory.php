...
if (isset($products)) {
    if ($products->count() > 0) {
        ...
        <script>
            (function ($) {
                $(window).load(function () {
                    ...
                    $(".blockSliderContent").mCustomScrollbar({
                        axis: "x",
        ...
        <table style="width: 750px; margin-top: 30px; margin-bottom: 30px;">
            ...
            <tr style="border: 1px solid #e0e0e0;" class="blockProd"><td style="border: 0;padding: 0px;">
                    <div class="blockSliderContent" style="overflow-x: auto; width: 750px;">
                        <ul class="ulSimilarItemContent item-list" style="padding: 0;">
                            <? foreach ($products as $key => $product): ?>
                                ...
                                include_component('category', 'products', array('slug' => $sf_request->getParameter('slug'), 'product' => $product, 'sf_cache_key' => $product->getId() . "-" . sfContext::getInstance()->getUser()->hasPermission('Show article') . "-line4-nolast-prodnum0" . "-" . $prodInCart . "-" . $prodInDesire . "-" . $prodInCompare . "", 'prodCount' => $prodCount, 'showarticle' => sfContext::getInstance()->getUser()->hasPermission('Show article'), 'prodNum' => 0, 'line4' => false, "productShowItems" => true, 'last' => false, "prodInCart" => $prodInCart, "prodInDesire" => $prodInDesire, "prodInCompare" => $prodInCompare));
                                ?>

                            <?php endforeach; ?>
                        </ul>
                    </div></td></tr>
            <tr style="background: #e0e0e0;color: #707070;border: 1px solid #e0e0e0;" class="blockProd"><td style="padding: 0px; height: 5px;"></td></tr>
        </table>
        <?
    }
}
...
/* if ($products->count() > 0) {
  ...
  rrApi.recomTrack('CategoryToItems', <?= $catId ?>, [<?= $prodId ?>]);
