...
<div class="c">
    <div class="content<?=($product['count'] == 0) ? ' notcount' : ''?>" >
      <?php //if ($product['count'] > 0): ?>
        ...
        ><?=$product['count'] > 0 ? 'В корзину' : 'Заказать'?></div>
      <?php //endif ?>
        ...
        <div class="title"><a class="gtm-link-item" href="/product/<?= $product['slug'] ?>"><? mb_internal_encoding('UTF-8'); ?><?= $product['name'] ?></a></div><?php /* mb_substr($product['name'], 0, 55)*/?>

        <div class="img-holder">
            <a href="/product/<?= $product['slug'] ?>" class="gtm-link-item">
                <img <?php if ($autoLoadPhoto) {
            echo 'src="/uploads/photo/thumbnails_250x250/' . $photo['filename'] . '"';
        } else {
            echo 'src="/images/pixel.gif" data-original="/uploads/photo/thumbnails_250x250/' . $photo['filename'] . '"';
        } ?> alt='<?= str_replace(array("'", '"'), "", $product['name']) ?>' title='<?= str_replace(array("'", '"'), "", $product['name']) ?>' class="thumbnails" />
...
<? if ($product['endaction'] != ""): ?>
        ...
        <div class="bottom-box">
            <div class="row">
<? if ($comment['countcomm'] > 0) { ?>
  <a class="gtm-link-item" href="/product/<?= $product['slug'] ?>/?comment=true#comments" class="rewiev">Отзывы: <?= $comment['countcomm'] ?></a>
<? } ?>
<? if (!$comment['countcomm'] ) {
  echo '&nbsp;';
} ?>
                <div class="stars">
...
<? } else { ?>
            ...
            <div class="bonus-box-price"><?php
                ...
                if ($product['bonuspay'] > 0) {
                    ...
                    ?>+<?= round(($product['price'] - $product['price'] * ($product['bonuspay'] > 0 ? $product['bonuspay'] : csSettings::get('PERSENT_BONUS_PAY')) / 100) * (($product['bonus'] > 0 ? $product['bonus'] : csSettings::get('persent_bonus_add')) / 100)) ?> бонусов
    <? }
$timer->addTime();
?>
            </div>
    ...
    <div class="preShowProduct-<?= $product['id'] ?> fadeBlock">
        <div  class="preShowProduct">
            <ul class="item-list" style="padding: 0px;"><li style="border:0px;padding: 0px;">
                    <div class="c">
                        <div class="content contentPreShowProduct-<?= $product['id'] ?><?=($product['count'] == 0)?' notcount' : '' ?>">

                        <?/*<div
                          class="silverButtonPreShow button-preshow-catalog"
                          onClick="
                            loadPreShow(<?= $product['id'] ?>, '<?=implode(', ', array_keys($products))?>', 0);
