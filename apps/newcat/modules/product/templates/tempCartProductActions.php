            ...
            <form id="senduser-<?= $product->getId() ?>" action="/product/<?= $product->getSlug() ?>/senduser" method="post">
                <div style="clear: both; color:#4e4e4e; text-align: left;">    
                    ...
                    <div style="width:100%; text-align: center;">
                        ...
                        <br />
                        <br />
                        <input type="submit" style="width: 198px; position: relative; left: 0px; top: 0px; margin: 5px 0px 7px 0px;" value="Отправить запрос" id="addToBasket4-<?= $product->getId() ?>">
                    </div>
    ...
    <div class="c">
        <div class="content">
            ...
            elseif ($num == 2)
                ...
                echo "green";
            ?>"><?= $action_name ?></div>
                <?= $product->getDiscount() > 0 ? '<span class="sale' . $product->getDiscount() . '" style="top: 70px;">-' . $product->getDiscount() . '%</span>' : ''; ?>
                <?= strtotime($product->getCreatedAt()) > time() - (csSettings::get('logo_new') * 24 * 60 * 60) ? '<span class="newProduct" style="top: 70px;"></span>' : ''; ?>
...
<? } else { ?>
                ...
                <div class="tools">
                    <a href="#" class="att"></a><?php if ($product->getCount() > 0): ?>
                        <a id="buttonId_<?= $product->getId() ?>" href="#" style="cursor: pointer;" class="red-btn small to-card" onClick="javascript:$.post('/cart/addtocart/<?= $product->getId() ?>', {quantity: 1}, function (data) {
    ...
    <? /*  addToCartAnim('Cart', '#photoimg_<?= $product->getId() ?>', true); */ ?>
                        ...
                        <a href="" style="cursor: pointer;" class="to-desire" onClick='javascript: addToCartAnim("Jel", "#photoimg_<?= $product->getId() ?>");
                                    $("#JelHeader").load("/cart/addtodesire/<?= $product->getId() ?>");'></a>

                    <?php else: ?>
                        <a id="addToBasket2-<?= $product->getId() ?>" class="red-btn to-card small" onclick="return hs.htmlExpand(this, {contentId: 'ContentToSend_<?= $product->getId() ?>', outlineType: 'rounded-white', wrapperClassName: 'draggable-header no-controlbar',
...
<?php endif; ?></dd>
    ...
    <? /*  addToCartAnim('Cart', '#photoimg_pr_<?= $product->getId() ?>'); */ ?>
                                                ...
                                                <a href="#" class="to-desire" onClick='javascript: addToCartAnim("Jel", "#photoimg_<?= $product->getId() ?>");
                                                            $("#JelHeader").load("/cart/addtodesire/<?= $product->getId() ?>");'></a>
                                            <?php else: ?>
