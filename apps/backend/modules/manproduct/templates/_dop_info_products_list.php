    ...
    <div>
        ...
        <div class="content">
            ...
            foreach ($dop_info as $dop_info_name):
                ?>
                <a onclick="$('#properties_<?= $dop_info_name->getId(); ?>').toggle('slow');
                        return false;" href="#"><span style="font-size:14px"><?= $dop_info_name->getName(); ?></span></a><br />
                    ...
                    foreach ($dop_info_values as $dop_info_value):
                        ...
                        <div align="left"><input type="checkbox" value="<?= $dop_info_value->getId(); ?>"  
                            <?php if ((array_search($dop_info_value->getId(), $form['dop_info_products_list']->getValue()) !== false and $form->isNew() === false)
                                    or ( isset($productRequest['dop_info_products_list']) and array_search($dop_info_value->getId(), $productRequest['dop_info_products_list']) !== false))
                                echo " checked=\"\""
                                ?>
        ...
        <div class = "content"><?php
                if (count($form['dop_info_products_list']->getValue()) > 0) {
                    $DopInfos = DopInfoTable::getInstance()->createQuery("d")->leftJoin("d.DopInfoCategory as dic")->where('d.id in(' . implode(",", $form['dop_info_products_list']->getValue()) . ')')->execute();
                    foreach ($DopInfos as $DopInfo) {
                        ?><div><?= $DopInfo->getDopInfoCategory()->getName() ?> - <?= $DopInfo->getValue() ?> <span style="cursor:pointer;font-weight: bold" onclick="$('input[value=<?= $DopInfo->getId() ?>][name=\'product[dop_info_products_list][]\']').removeAttr('checked');
