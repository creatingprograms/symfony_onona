    ...
    <?php else: ?>
        ...
        <script language="javascript">
            $(document).ready(function() { 
                $("#articleTable").tableDnD(
                {
                    ...
                    onDrop: function(table, row) 
                    {
                        ...
                        {
                            type: "POST",
                            url: "/backend_dev.php/article/sortChange",

                            timeout: 5000,

                            data: "table=article&rowId=" + row.id + "&nextSortOrder=" + next + "&currentSortOrder=" + row.title + "&rowsList=" + rowsList,
            ...
            function changePublic(id){     
                ...
                {
                    type: "POST",
                    url: "/backend_dev.php/article/publicChange",
            ...
            function changeRelated(id){     
                ...
                {
                    type: "POST",
                    url: "/backend_dev.php/article/relatedChange",
        ...
        <table cellspacing="0" id="articleTable">
            ...
            <tbody>
                <?php foreach ($pager->getResults() as $i => $article): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
                    <tr class="sf_admin_row <?php echo $odd ?>" id="<?= $article->getId() ?>" title="<?= $article->get("position") ?>">
