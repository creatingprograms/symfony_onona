...
class dopFormPage {
    ...
    static function getFormSexolog($errorCap) {
        ...
        $html = <<<HERE
    <form action="/vopros-seksologu" method="post" id="support">
<table calss="noBorder" class="tableRegister">
  ...
  <td><input type="text" name="sub" value="" id="sub"></td>
</tr>
                </tbody>
  ...
  <td><textarea name="msg" value="" id="msg"></textarea></td>
</tr>
                </tbody>
...
HERE;
        $html.='<td><img border="0" style="float: left; margin-right: 10px;" src="/captcha/supportcaptcha.php?' . session_name() . '=' . session_id() . '"><input type="text" name="code" value="" id="code"><br>';
        ...
        * - поля, отмеченные * обязательны для заполнения.<br>
        <div style="margin: auto; width: 147px;"><a class = "red-btn colorWhite" href = "#" onclick = "$('#support').submit(); return false;"><span style = "width: 120px;">Отправить</span></a></div>
        </form>
HERE;
        return $html;
    }
...
HERE;
    ...
    static function getFormRegformvideo($errorCap) {
        ...
        mb_internal_encoding('UTF-8');
        $html = '
    <form action="/forma-registracii-dlya-video" method="post" id="support">
<table calss="noBorder" class="tableRegister">
                <tbody>
                    <tr>
  <th><label for="name">Ваше имя/псевдоним:*</label></th>
