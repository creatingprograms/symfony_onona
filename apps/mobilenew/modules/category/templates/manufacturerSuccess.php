...
slot('metaTitle', str_replace("{name}", $manufacturer['name'], csSettings::get('titleCategory')) . (sfContext::getInstance()->getRequest()->getParameter('page', 1) > 1 ? " - Страница " . sfContext::getInstance()->getRequest()->getParameter('page', 1) : "") . " – Секс шоп Он и Она");
slot('metaKeywords', str_replace("{name}", $manufacturer['name'], csSettings::get('titleCategory')));
slot('metaDescription', str_replace("{name}", $manufacturer['name'], csSettings::get('titleCategory')));
?>
...
<div id="productsShow">
    ...
    ?>
    <?php
    if (@mb_strlen(strip_tags($manufacturer['content'])) > 0) {
        ?>
        <div class="description">
            <div class="top">
                <?= $manufacturer['name'] ?>
            </div>
            <?= mb_substr(strip_tags($manufacturer['content']), 0, 150) ?>
            ...
            <div class="spoiler">
                <?= @mb_substr(strip_tags($manufacturer['content']), 150, mb_strlen(strip_tags($manufacturer['content']))) ?>
            </div>
        </div>
        <?php
    }
