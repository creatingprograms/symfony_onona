...
<ul>';
	...
	<div class="news-frame">
		...
		<ul class="news-list">';
            foreach ($news as $new) {
                $newsBlock.='<li style="color: #707070;">
				<div class="title">
					<a href="/news/' . ($new->getSlug()) . '">' . ($new->getName()) . '</a></div>
    ...
    public function blockAddComment() {
        if (substr_count($this->page['content'], "{commentsMagazBlock}") > 0) {
            if (isset($_POST["cName"])) {
                if ($_POST["cName"] != "") {
                    ...
                    if ($captcha->getVal() == $_POST["cText"]) {
                        ...
                        $newComments->setPageId($this->page['id']);
            ...
            $commMagazBlock = "";
            $comments = CommentsTable::getInstance()->createQuery()->where("is_public='1' and page_id = " . $this->page['id'])->execute();
 ...
 <div class=\"add-coment\" style=\"display:none;width: 500px; margin: 24px auto;\" id=\"commentDiv\">
            <form id=\"commentForm\" method=\"post\" action=\"/" . ($this->page['slug']) . "/addcomm\" name=\"comment\">
                <fieldset>
...
";
            ...
            if (isset($_POST["cText"]))
                if (@$captchaVal == $_POST["cText"] and $_POST["cText"] != "") {
                    $commMagazBlock = "Отзыв успешно добавлен. После проверки модератором, он станет доступнен для просмотра. <a href=\"/" . ($this->page['slug']) . "\">Вернуться к просмотру магазина</a><br /><br />";
                }
            if ($comments->count() > 0) {
...
<table width=\"100%\" border=\"0\" id=\"commentsTable\" align=\"center\" class=\"noBorder\">
    <a name=\"отзывы\"></a>";
...
";
            $this->page['content'] = str_replace('{commentsMagazBlock}', $commMagazBlock, $this->page['content']);
        }
        ...
        if (substr_count($this->page['content'], "{commentsPageBlock}") > 0) {
            if (isset($_POST["cName"])) {
                $captcha = CaptchaTable::getInstance()->createQuery()->where("subid='" . session_id() . "' and type='k'")->fetchOne();

                if ($captcha->getVal() == $_POST["cText"]) {
                    $this->page['content']=PageTable::getInstance()->find(286)->getContent() . '<br /><a href="/' . $this->page['slug'] . '">Вернуться к просмотру страницы - ' . $this->page['name'] . '.</a>';
                    $newComments = new Comments();
                    $newComments->setPageId($this->page['id']);
                    $newComments->setUsername($_POST["cName"]);
                    $newComments->setMail($_POST["cEmail"]);
